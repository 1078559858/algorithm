...
class RandomController extends Controller {
  constructor(ctx) {
    ...
    this.createRule = {
      ...
      // random: { type: 'string', required: true, allowEmpty: false }
    }
  }
  ...
  async create() {
    ...
    // 调用 Service 进行业务处理
    const res = await service.random.create(payload)
    // 设置响应内容和响应状态码
  ...
  async update() {
    ...
    const payload = ctx.request.body || {}
    // 调用 Service 进行业务处理
    await service.random.update(id, payload)
    // 设置响应内容和响应状态码
    ctx.helper.success({ctx})
  }
  ...
  async show() {
    ...
    const { id } = ctx.params
    // 调用 Service 进行业务处理
    const res = await service.random.show(id)
    // 设置响应内容和响应状态码
    ctx.helper.success({ctx, res})
  }
