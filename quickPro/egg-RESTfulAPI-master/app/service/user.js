...
class UserService extends Service {
  ...
  async create(payload) {
    ...
    const role = await service.role.showByName(payload.role)
    if (!role) {
  ...
  // show======================================================================================================>
  async show(_id) {
    const user = await this.ctx.service.user.findByMobile(_id)
    if (!user) {
      this.ctx.throw(404, 'user not found')
    }
    return this.ctx.model.User.find({mobile:_id}).populate('role')
  }
